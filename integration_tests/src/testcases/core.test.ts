...
describe('Core', () => {
  ...
  it('instantiate allowlist', async () => {
    ...
    const res = await client.upload(
      ...
      Uint8Array.from(
        fs.readFileSync(
          join(__dirname, '../../../artifacts/maxbtc_neutron_allow_list.wasm'),
        ),
      ),
      1.5,
    );
  ...
  it('instantiate liquidation buffer', async () => {
    ...
    const res = await client.upload(
      ...
      Uint8Array.from(
        fs.readFileSync(
          join(
            __dirname,
            '../../../artifacts/maxbtc_neutron_liquidation_buffer.wasm',
          ),
        ),
      ),
      1.5,
    ...
    const instantiateRes =
      await MaxbtcNeutronLiquidationBuffer.Client.instantiate(
        client,
        account.address,
        res.codeId,
        {
    ...
    context.liquidationBufferContractClient =
      new MaxbtcNeutronLiquidationBuffer.Client(
        client,
  ...
  it('should handle the full withdrawal cycle: withdraw -> process -> fund collector -> claim', async () => {
    ...
    console.log(
      'Seeding account with additional maxBTC for withdrawal tests...',
    );
    ...
    const initialMaxBtcBalance = BigInt(
      (
    ...
    const finalMaxBtcBalance = BigInt(
      (
        await client.getBalance(
          account.address,
          `factory/${coreContractAddress}/maxbtc`,
    ...
    expect(finalMaxBtcBalance).toEqual(
      initialMaxBtcBalance - BigInt(withdrawAmount),
    );
    ...
    const redemptionTokenBalance = (
      await client.getBalance(account.address, redemptionTokenDenom)
    ).amount;
    expect(redemptionTokenBalance).toEqual(withdrawAmount);

    console.log(
      `Successfully withdrew and received ${withdrawAmount} of ${redemptionTokenDenom}`,
    );

    // STEP 2: Wait for batch to be processable
    const batchActiveDuration = 10; // As set in instantiation
    console.log(
      `Waiting for ${batchActiveDuration + 1} seconds for the batch to expire...`,
    );
    await new Promise((resolve) =>
      setTimeout(resolve, (batchActiveDuration + 1) * 1000),
    );

    // STEP 3: Process the active batch
    console.log('Processing the active batch...');
    const processRes = await coreContractClient.processActiveBatch(
      account.address,
      'auto',
    );
    ...
    console.log(
      `Batch ${activeBatch.batch_id} is now in 'withdrawing' state, requesting ${withdrawingBatch.btc_requested} untrn.`,
    );

    // STEP 4: Simulate Collector Funding & Finalize Batch
    const amountNeeded = withdrawingBatch.btc_requested;
    console.log(
      `Sending ${amountNeeded} untrn to collector contract ${collectorContractAddress}...`,
    );
    ...
    await coreContractClient.processCache(
      account.address,
      1.5,
      'process cache',
    );
    ...
    const finalizedBatch = await coreContractClient.queryFinalizedBatch({
      batch_id: withdrawingBatch.batch_id,
    });
    expect(finalizedBatch).toBeTruthy();
    expect(finalizedBatch.collected_amount).toEqual(amountNeeded);
    const postFinalizeWithdrawingBatch =
      await coreContractClient.queryWithdrawingBatch();
    ...
    console.log(
      `Claiming ${amountNeeded} untrn from finalized batch ${finalizedBatch.batch_id}...`,
    );
    ...
    const finalRedemptionTokenBalance = (
      await client.getBalance(account.address, redemptionTokenDenom)
    ).amount;
    ...
    const transferEvent = tx.events.find(
      (e) =>
        e.type === 'transfer' &&
        e.attributes.some(
          (attr) => attr.key === 'recipient' && attr.value === account.address,
        ),
    );
    expect(transferEvent).toBeDefined();
    const receivedAmountAttr = transferEvent.attributes.find(
  ...
  it('should handle flushDeposits and FSM state transitions correctly', async () => {
    const {
      ...
      account,
      coreContractAddress,
      liquidationBufferContractAddress,
      pumpContractAddress,
      aumOracleContractClient,
    } = context;

    // Ensure core contract has funds.
    console.log('Depositing funds to prepare for flush test...');
    await coreContractClient.deposit(
      account.address,
      { recipient: account.address },
      'auto',
      'flush setup',
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, (flushPeriod + 1) * 1000),
    );

    // Get pre-flush state.
    const coreBalanceBefore = await client.getBalance(
      coreContractAddress,
      'untrn',
    );
    const bufferBalanceBefore = await client.getBalance(
      liquidationBufferContractAddress,
      'untrn',
    );
    const pumpBalanceBefore = await client.getBalance(
      pumpContractAddress,
      'untrn',
    );
    ...
    await aumOracleContractClient.updateConfig(
      account.address,
      { aum: '0' },
      'auto',
    );
    await context.liquidationBufferContractClient.updateConfig(
      account.address,
      { owned_btc: '0', owned_maxbtc: '0' },
      'auto',
    );

    // Calculation based on contract logic
    const totalAUM =
      BigInt(coreBalanceBefore.amount) + BigInt(bufferBalanceBefore.amount);
    ...
    const coreBalanceAfter = await client.getBalance(
      coreContractAddress,
      'untrn',
    );
    const bufferBalanceAfter = await client.getBalance(
      liquidationBufferContractAddress,
      'untrn',
    );
    const pumpBalanceAfter = await client.getBalance(
      pumpContractAddress,
      'untrn',
    );

    expect(coreBalanceAfter.amount).toEqual('0');
    expect(BigInt(bufferBalanceAfter.amount)).toEqual(
      BigInt(bufferBalanceBefore.amount) + toBuffer,
    );
    expect(BigInt(pumpBalanceAfter.amount)).toEqual(
      BigInt(pumpBalanceBefore.amount) + toPump,
    );
    console.log(
      `Flush successful. Sent ${toBuffer} to buffer, ${toPump} to pump.`,
    );
    ...
    const finalDepositRes = await coreContractClient.deposit(
      account.address,
      { recipient: account.address },
      'auto',
      'testing idle state',
  ...
  it('should allow deposits and withdrawals while in the FLUSHING state', async () => {
    ...
    const flushPeriod = 10; // As set in instantiation
    console.log(
      `Waiting for ${flushPeriod + 1} seconds for the deposit to be flushable...`,
    );
    await new Promise((resolve) =>
      setTimeout(resolve, (flushPeriod + 1) * 1000),
    );
    ...
    const flushRes = await coreContractClient.flushDeposits(
      account.address,
      'auto',
    );
    ...
    console.log(
      `Depositing ${depositAmountDuringFlush} untrn while contract is flushing...`,
    );
    ...
    console.log(
      `Withdrawing ${withdrawAmountDuringFlush} maxBTC while contract is flushing...`,
    );
    ...
    console.log(
      'Deposit and withdrawal were successfully queued during the flush.',
    );
    ...
    console.log(
      `Simulating flush finalization by updating AUM oracle with ${flushedAmount}...`,
    );
    await aumOracleContractClient.updateConfig(
      account.address,
      { aum: flushedAmount },
    ...
    console.log(
      'Waiting for another flush period to test the post-flush state...',
    );
    await new Promise((resolve) =>
      setTimeout(resolve, (flushPeriod + 1) * 1000),
    );

    const coreBalanceBeforeNextFlush = await client.getBalance(
      coreContractAddress,
      'untrn',
    );
    ...
    await coreContractClient.flushDeposits(account.address, 'auto');
    const coreBalanceAfterNextFlush = await client.getBalance(
      coreContractAddress,
      'untrn',
    );
    expect(coreBalanceAfterNextFlush.amount).toEqual('0');
    console.log(
      'Successfully flushed the deposit that was made during the previous flush cycle.',
    );
    ...
    await aumOracleContractClient.updateConfig(
      account.address,
      { aum: (flushedAmount + 36000).toString() },
      'auto',
    );
  ...
  it('should reject new state transitions while not in IDLE state', async () => {
    ...
    await coreContractClient.deposit(
      ...
      'auto',
      'setup',
      [{ denom: 'untrn', amount: '50000' }],
    );
    await coreContractClient.withdraw(account.address, 'auto', 'setup', [
      {
        denom: `factory/${context.coreContractAddress}/maxbtc`,
        amount: '1000',
      },
    ]);
    ...
    console.log(
      `Waiting ${batchActiveDuration + 1}s for batch to become processable...`,
    );
    await new Promise((resolve) =>
      setTimeout(resolve, (batchActiveDuration + 1) * 1000),
    );
    ...
    console.log(
      'Attempting to call flushDeposits from WITHDRAWING state (expected to fail)...',
    );
    await expect(
      coreContractClient.flushDeposits(account.address, 'auto'),
    ).rejects.toThrow(/This FSM transition is not allowed/);
    ...
    await client.sendTokens(
      account.address,
      collectorContractAddress,
      [{ denom: 'untrn', amount: withdrawingBatch.btc_requested }],
      1.5,
    );
    await coreContractClient.processCache(account.address, 1.5);
    ...
    console.log(
      'Attempting to call processActiveBatch from FLUSHING state (expected to fail)...',
    );
    await expect(
      coreContractClient.processActiveBatch(account.address, 'auto'),
    ).rejects.toThrow(/This FSM transition is not allowed/);
