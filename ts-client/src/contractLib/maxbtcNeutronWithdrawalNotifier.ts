...
export class Client {
  ...
  queryAmount = async(args: AmountArgs): Promise<Uint128> => {
  ...
  submitAmount = async(sender:string, args: SubmitAmountArgs, fee?: number | StdFee | "auto", memo?: string, funds?: Coin[]): Promise<ExecuteResult> =>  {
          if (!isSigningCosmWasmClient(this.client)) { throw this.mustBeSigningClient(); }
    return this.client.execute(sender, this.contractAddress, this.submitAmountMsg(args), fee || "auto", memo, funds);
  }
  submitAmountMsg = (args: SubmitAmountArgs): { submit_amount: SubmitAmountArgs } => { return { submit_amount: args }; }
