...
class Client {
    ...
    queryAmount = async (args) => {
        return this.client.queryContractSmart(this.contractAddress, { amount: args });
    };
    ...
    submitAmount = async (sender, args, fee, memo, funds) => {
        if (!isSigningCosmWasmClient(this.client)) {
            throw this.mustBeSigningClient();
        }
        return this.client.execute(sender, this.contractAddress, this.submitAmountMsg(args), fee || "auto", memo, funds);
    };
    submitAmountMsg = (args) => { return { submit_amount: args }; };
